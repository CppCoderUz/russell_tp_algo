...
from utils import real_or_int, Price
...
class TransportationSolveGUI:
    ...
    def __init__(self, solution_class, point_list: list, demand_name: str, offer_name: str) -> None:
        ...
        self.window = tk.Tk()
        self.window.title("Russel usulida transport muamosini hal qilish dasturi .")
    ...
    def demand_label_draw(self):
        for i in range(self.demand_size):
            self.label_demand.append(CustomButton(
                ...
                position_y=50, state=tk.DISABLED, font=('Arial 10')
            ))
    ...
    def buttons_draw(self):
        self.button_demand_plus = CustomButton(self.window, text=' + ', fg='green', font=('Arial 20'), width=70, height=40, position_x=40, position_y=0, command=self.add_demand)
        self.button_demand_minus = CustomButton(self.window, text=' - ', fg='red', font=('Arial 20'), width=70, height=40, position_x=120, position_y=0, command=self.subtraction_demand)
        
        self.button_offer_plus = CustomButton(self.window, text=' + ',fg='green', font=('Arial 20'), width=40, height=70, position_x=0, position_y=40, command=self.add_offer)
        self.button_offer_minus = CustomButton(self.window, text=' - ', fg='red', font=('Arial 20'), width=40, height=70, position_x=0, position_y=120, command=self.subtraction_offer)


        self.solve_button = CustomButton(self.window, text=' Hisoblash ', fg='black', font=('Arial 13'), width=150, height=40, position_x=200, position_y=0, command=self.solve)
    ...
    def solve(self):
        ...
        for i in range(len(self.inputs)):
            ...
            for j in range(len(self.inputs[i])):
                ...
                if float(text) < 0:
                    return self.error_view(error=''' Manfiy son kiritish mumkin emas ''')
                try:
        ...
        try:
            points = self.solution_class(demand=talab, offer=taklif, costs=price_matrix).solve()
            self.solve_view(point_list=points, price_matrix=price_matrix)
    ...
    def error_view(self, error: str):
        new_window = tk.Tk()
        new_window.title(" Xatolik !")
        new_window.geometry("400x150+500+300")
        def close_new_window():
            new_window.destroy()
        CustomButton(new_window, text=error, width=400, height=150, position_x=0, position_y=0, font=('Arial 14'), fg='red', command=close_new_window)
        new_window.mainloop()
    ...
    def solve_view(self, point_list: list[Price], price_matrix: list[list[int]]):
        ...
        new_window = tk.Tk()
        new_window.title(" Muammo yechimi ")
        width = self.demand_size * 100 + 100
        height = self.offer_size * 50 + 200
        if (width < len(text) * 10 + 60): 
            width = len(text) * 10 + 60

        new_window.geometry(f"{width}x{height}+150+150")
        CustomButton(new_window, width=width - 40, height=80, position_x=20, position_y=20, state=tk.DISABLED, text=text, font=('Arial 14'))
        ...
        for i in range(self.offer_size):
            ...
            for j in range(self.demand_size):
                ci = CustomInput(new_window, width=100, height=50, position_x=40+j*100, position_y=140+i*50)
